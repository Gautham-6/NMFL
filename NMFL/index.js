...
// const us1 = new HospitalAdmin({
//   emailid:"ydantano@asu.edu",
//   password:"12345",
//   hospitalid:"ok"
// });
...
try {
  ...
  app.get('/adminlogin', function (req, res) {
    ...
    app.post("/adminlogin",async function(req,res){
      ...
      var apassword = req.body.adminpassword;
...
//   // displaying filter values in console
...
//   if (req.body.slider){
//
//     const response = await fetch('http://api.openweathermap.org/geo/1.0/reverse?lat='+req.body.lattextvalue+'&lon='+req.body.longtextvalue+'&appid='+process.env.OPENWMAP, {method: 'GET'});
//     const data = await response.json();
//     console.log(data);
//     cityname=data[0].name;
//     latset= req.body.lattextvalue;
//     lngset = req.body.longtextvalue;
//     searchhint = data[0].name+", "+data[0].state+", "+"USA"
//   }
//
//   var latlng={
//     lat:latset,
...
//   const bearer = "Bearer "+"LkNiA3LMLvx7FE470fICLn1uvoss18v8bqZklvo8VBswqESoaoRVNYuraUDqO5d4NHQwFgLOWBY-D8VB7OvXoUaMtw8-mO4LQgI62U4q8-GcYb1Ja4mmkJGkNFZ-Y3Yx"
//   //var apiRequest = 'https://api.yelp.com/v3/businesses/search?location='+cityname+'&categories=hospitals&categories=pharmacy&open_now=true&sort_by=best_match&limit=15'
//   var apiRequest = 'https://api.yelp.com/v3/businesses/search?location='+cityname+'&categories=hospitals'
...
//   // filter category -
//   //hospital category
//   // sort by - rating , best best_match
//   // lmit results by
//
//   res.render("index", {state:state, latlng:latlng, city: cityname, data:dataHospital.businesses, searchhint: searchhint});
//
//   });
  ...
  app.post("/searchhospitals", async function(req,res){
    try{
      ...
      if(enteredcity.split(",").length <= 2){
        // res.render("homepage",{cityname:enteredcity.trim(),cityerror:"Please select a valid city - Not the state"});
...
try{
  if(req.session.partneremail){
    ...
    //console.log(findfeaturedetail.length+"----"+findfeaturedetail[0].FeatureDescription)
...
try{
  ...
  app.post('/hospitalDoctorEdit',async function(req,res){
    try{
     ...
     if((findDoctorDetails.length>0)&&((findDoctorDetails[0].DoctorSpecialization!=olddoctorDescription)||(findDoctorDetails[0].DoctorPhonenumber!=olddoctorphone))){
       const errorMessage = "The Doctor already exists";
       res.redirect(`/dashboard?error=${errorMessage}`);
     }
     else{
     DoctorDetails.updateOne({hospitalid:req.session.hospitalid,DoctorName:olddoctorName,DoctorSpecialization:olddoctorDescription,DoctorEmail:olddoctorEmail,DoctorPhonenumber:olddoctorphone},
         {
           $set:{
             DoctorName:doctorName,
             DoctorSpecialization:doctorSpecialization,
  ...
  res.render("error",{errormessage:"There is an error trying to register your hospital. Kindly try again later.",pageredirect:"/homepage"})
    ...
    app.post('/forgotpassword', async function (req, res) {
      ...
      if(email1 && password && cnfpassword){
        if(password==cnfpassword){
          ...
          console.log(otp);
          console.log(fetchOTP.hospitalOTP);
    ...
    //   });
      ...
      app.post("/hospital",async function(req,res){
        //console.log(req.body.hospitaltag);
        try{
    ...
    app.post('/predict',function(req,res){
      ...
      console.log(predictionResult2);
      console.log(predictionResult3);
      res.send({
